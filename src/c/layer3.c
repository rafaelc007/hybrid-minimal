...
void layer3_update(Layer *layer, GContext *ctx, struct tm *current_time,
  ...
  // -- Middle: Weather (icon + temp only) --
  char weather_str[8];
  if (weather_temp == -999) {
    snprintf(weather_str, sizeof(weather_str), "--");
  } else {
    snprintf(weather_str, sizeof(weather_str), "%d\xc2\xb0", weather_temp);
  }
  ...
  if (icon_weather) {
    ...
    int16_t group_x = mid_slot.origin.x + (slot_w - group_w) / 2;
    int16_t icon_y  = mid_slot.origin.y + (slot_h - icon_size.h) / 2;
    gdraw_command_image_draw(ctx, icon_weather, GPoint(group_x, icon_y));
    graphics_draw_text(ctx, weather_str,
                       fonts_get_system_font(FONT_KEY_GOTHIC_18),
                       GRect(group_x + icon_size.w + gap, mid_slot.origin.y, text_w, slot_h),
